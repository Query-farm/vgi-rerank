"""In-process VGI invocation helpers for the rerank worker test suite.
...
fastembed cross-encoder is available locally, so model-download tests skip cleanly
...
def model_available() -> bool:
    """Whether the default fastembed cross-encoder can be loaded (for skip guards).

    True once the ONNX model is cached locally (or downloadable). Lets the
    model-backed tests self-skip on a bare, offline checkout while still running
